...
class Subforum():
    ...
    def get_postdate(self, postid):
        ...
        cdate = datetime.datetime.strptime(
            self.postdict[postid]['creationdate'], "%Y-%m-%dT%H:%M:%S.%f")
    ...
    def get_acceptedanswer(self, postid):
        ...
        if postid not in self.postdict:
            raise KeyError(
                postid + " is not a valid post id for this subforum.")
    ...
    def get_acceptedanswer_date(self, answerid):
        ...
        if self.answerdict[answerid]['acceptedanswerdate'] == 0:
            ...
            adate = datetime.datetime.strptime(
                self.answerdict[answerid]['acceptedanswerdate'], "%Y-%m-%dT%H:%M:%S.%f")
    ...
    def get_answerdate(self, answerid):
        ...
        cdate = datetime.datetime.strptime(
            self.answerdict[answerid]['creationdate'], "%Y-%m-%dT%H:%M:%S.%f")
        return str(cdate.year) + '-' + cdate.strftime('%m') + '-' + cdate.strftime('%d')
    ...
    def get_commentdate(self, commentid):
        ...
        cdate = datetime.datetime.strptime(
            self.commentdict[commentid]['creationdate'], "%Y-%m-%dT%H:%M:%S.%f")
        return str(cdate.year) + '-' + cdate.strftime('%m') + '-' + cdate.strftime('%d')
    ...
    def get_user_joindate(self, userid):
        ...
        cdate = datetime.datetime.strptime(
            self.userdict[userid]['date_joined'], "%Y-%m-%dT%H:%M:%S.%f")
        return str(cdate.year) + '-' + cdate.strftime('%m') + '-' + cdate.strftime('%d')
    ...
    def get_user_lastaccess(self, userid):
        ...
        cdate = datetime.datetime.strptime(
            self.userdict[userid]['lastaccessdate'], "%Y-%m-%dT%H:%M:%S.%f")
        return str(cdate.year) + '-' + cdate.strftime('%m') + '-' + cdate.strftime('%d')
    ...
    def change_to_default_stopwords(self, stopwordset='middle'):
        ...
        if stopwordset == 'nltk':
            if self.__nltk_stopwords != []:
                ...
                self.__stopwords = [u'i', u'me', u'my', u'myself', u'we', u'our', u'ours', u'ourselves', u'you', u'your', u'yours', u'yourself', u'yourselves', u'he', u'him', u'his', u'himself', u'she', u'her', u'hers', u'herself', u'it', u'its', u'itself', u'they', u'them', u'their', u'theirs', u'themselves', u'what', u'which', u'who', u'whom', u'this', u'that', u'these', u'those', u'am', u'is', u'are', u'was', u'were', u'be', u'been', u'being', u'have', u'has', u'had', u'having', u'do', u'does', u'did', u'doing', u'a', u'an', u'the', u'and', u'but', u'if', u'or', u'because', u'as',
    ...
    def _deal_with_code(self, s, maxcodelength):
        ...
        # re.M for multiline matching not necessary?
        codes = re.findall(codepat, s)
        n = 0
        newcodes = []
        for c in codes:
    ...
    def url_cleaning(self, s):
        ...
        posduppat = re.compile(
            r'<blockquote>(.|\n)+?Possible Duplicate(.|\n)+?</blockquote>', re.MULTILINE)
        s = re.sub(posduppat, '', s)

        s = re.sub(r'<a[^>]+stackexchange[^>]+>([^<]+)</a>',
                   r'stackexchange-url ("\1")', s)
        s = re.sub(r'<a[^>]+stackoverflow[^>]+>([^<]+)</a>',
                   r'stackexchange-url ("\1")', s)
    ...
    def _remove_stopwords(self, s):
        ...
        # The NLTK tokenizer cuts things like 'cannot' into 'can' and 'not'.
        ...
        for w in words:
            ...
            if words_split[counter] == w:  # word was correctly split
                ...
                counter += 1
                prevw_in_split = True
            # previous word was fine, but this is the first part of a wrongly split word.
            elif prevw_in_split:
                filteredwords.append(w)
    ...
    def _remove_tags(self, s):
        ...
        s = re.sub(
            r"<a href=\"https?://[a-z]+\.stackexchange\.com[^\"]+\">([^<]+)</a>", r"\1", s)
        ...
        s = re.sub(
            r"https?://([a-z]+\.)?stackexchange\.com[^ ]+", "stackexchange-url", s)
    ...
    def _general_cleaning(self, s, codes, remove_punct=False):
        ...
        for url in set(urls):
            if re.search(r'\)', url) and not re.search('\(', url):
                url = re.sub(r'\).*$', '', url)
            # Get rid of backslashes because else we get regex problems when trying to put the URLs back.
            if re.search(r'\\', url):
                url = re.sub(r'\\', '/', url)
        ...
        if remove_punct:
            ...
            if l:
                for m in l:
                    ...
                    s = re.sub(r'\.', '', s)
                    # place dots back in abbreviations
                    s = re.sub(newabbr, newbit, s)
            else:
                s = re.sub(r'\.', ' ', s)
            s = re.sub(r',', ' ', s)
            s = re.sub(r'\?', ' ', s)
    ...
    def split_for_classification(self, outputdir='.'):
        ...
        # Should not happen with the supplied forums, only with the smaller ones I'm sometimes testing with.
        if self.cutoffdate == 'nogood':
        ...
        withdups = []
        withoutdups = []
        testfile = open(outputdir + '/' + self.cat +
                        '_testpairs_large.txt', 'w')
        for i, postid in enumerate(testposts):
            dups = self.get_duplicates(postid)
            # combine each post with older posts.
            combine_with = testposts[i+1:]
        ...
        # Take ten times the nr of duplicate pairs as non-duplicate pairs. The rest of the non-duplicate pairs will not be used in the small test set.
        nrofnondups_forsmallset = len(withdups) * 10
        withoutdups_for_smallset = []
        while nrofnondups_forsmallset > 0:
            # randrange very conveniently excludes the last element of the list.
            pos = randrange(len(withoutdups))
            picked_nondup = withoutdups[pos]
            # Replace picked element with last one in the list
            withoutdups[pos] = withoutdups[-1]
            del withoutdups[-1]  # Cheap removal of element
            withoutdups_for_smallset.append(picked_nondup)
            nrofnondups_forsmallset -= 1

        testfile = open(outputdir + '/' + self.cat +
                        '_testpairs_small.txt', 'w')
        for tup in withdups:
        ...
        trainfile1 = open(outputdir + '/' + self.cat +
                          '_trainpairs_01.txt', 'w')
        trainfile2 = open(outputdir + '/' + self.cat +
                          '_trainpairs_02.txt', 'w')
        trainfile3 = open(outputdir + '/' + self.cat +
                          '_trainpairs_03.txt', 'w')
        trainfile4 = open(outputdir + '/' + self.cat +
                          '_trainpairs_04.txt', 'w')
        trainfile5 = open(outputdir + '/' + self.cat +
                          '_trainpairs_05.txt', 'w')
        trainfile6 = open(outputdir + '/' + self.cat +
                          '_trainpairs_06.txt', 'w')
        trainfile7 = open(outputdir + '/' + self.cat +
                          '_trainpairs_07.txt', 'w')
        trainfile8 = open(outputdir + '/' + self.cat +
                          '_trainpairs_08.txt', 'w')
        trainfile9 = open(outputdir + '/' + self.cat +
                          '_trainpairs_09.txt', 'w')
        trainfile10 = open(outputdir + '/' + self.cat +
                           '_trainpairs_10.txt', 'w')

        filenrs = [trainfile1, trainfile2, trainfile3, trainfile4, trainfile5,
                   trainfile6, trainfile7, trainfile8, trainfile9, trainfile10]
        dupindex = 0
        nondupindex = 0
        ...
        for i, postid in enumerate(trainposts):
            dups = self.get_duplicates(postid)
            # combine each post with older posts.
            combine_with = trainposts[i+1:]
            for otherpostid in combine_with:
                if otherpostid in dups:
                    filenrs[dupindex].write(
                        postid + ' ' + otherpostid + ' 1\n')  # 1 = duplicate pair
                    if dupindex == 9:
                        dupindex = 0
                    else:
                        dupindex += 1
                    ...
                    filenrs[nondupindex].write(
                        postid + ' ' + otherpostid + ' 0\n')
                    if nondupindex == 9:
                        nondupindex = 0
                    else:
                        nondupindex += 1
    ...
    def _find_cutoff_date(self):
        ...
        for thres in datelist:
            if t[thres]['test'] >= 200:
                if t[thres]['train'] >= 4 * t[thres]['test']:
                    datum = str(thres.year) + '-' + \
                        str(thres.month) + '-' + str(thres.day)
                    allcandidates.append(
                        (datum, t[thres]['test'], t[thres]['train'], t[thres]['lost']))
            ...
            for thres in datelist:
                if t[thres]['test'] >= 100:
                    if t[thres]['train'] >= 4 * t[thres]['test']:
                        ...
                        allcandidates.append(
                            (datum, t[thres]['test'], t[thres]['train'], t[thres]['lost']))
                        # One candiDATE is enough: the one with the highest nr of duplicate pairs in the test set.
                        break
        ...
        if allcandidates != []:
            ...
            # No cutoff candiDATE could be found that satisfies our constraints. Won't happen in the chosen subforums.
            self.cutoffdate = 'nogood'
    ...
    def average_ndcg_at(self, scorefile, cutoff=5, include_related_posts=False):
        ...
        all_rankings, all_relevantdocs, all_relateddocs = self._read_scorefile(
            scorefile)
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            # or (relateddocs != [] and include_related_posts): # We only compute the score for queries that actually do have duplicates in the indexed set.
            if relevantdocs != []:
                ...
                if include_related_posts:
                    relevance_judgements = map(lambda x: 1.0 if x in relevantdocs else (
                        0.5 if x in relateddocs else 0.0), ranking)
                else:
                    relevance_judgements = map(
                        lambda x: 1.0 if x in relevantdocs else 0.0, ranking)

                DCG = self._get_DCG(relevance_judgements)
                I = self._get_I(cutoff, include_related_posts,
                                relevantdocs, relateddocs)
    ...
    def average_recall_at(self, scorefile, cutoff=None, include_related_posts=False):
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:
                ...
                if cutoff:
                    ranking = ranking[:cutoff]
                relevant_in_ranking = len(
                    [w for w in relevantdocs if w in ranking])
                relevant_to_find = float(len(relevantdocs))
                if include_related_posts:
                    half_relevant_in_ranking = len(
                        [w for w in relateddocs if w in ranking]) / 2.0
    ...
    def average_precision_at(self, scorefile, cutoff=5, include_related_posts=False):
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:
                ...
                if cutoff:
                    ranking = ranking[:cutoff]
                # relevant docs is most likely shorter than ranking, so it is fastest this way around.
                relevant_in_ranking = len(
                    [w for w in relevantdocs if w in ranking])
                if include_related_posts:
                    half_relevant_in_ranking = len(
                        [w for w in relateddocs if w in ranking]) / 2.0
                    relevant_in_ranking += half_relevant_in_ranking
                precision_at_list.append(
                    relevant_in_ranking/float(len(ranking)))
    ...
    def mean_average_precision(self, scorefile, include_related_posts=False):
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:
                ...
                if include_related_posts:
                    av_p = self._average_precision(
                        ranking, relevantdocs, relateddocs=relateddocs)
                else:
            ...
            if relevantdocs != []:
                ...
                for i, v in enumerate(ranking):
                    if v in relevantdocs:
                        ...
                        # non relevant candidate. Do nothing, because won't add anything to sum.
                        pass
    ...
    def _average_precision(self, ranking, relevantdocs, relateddocs=[]):
        ...
        for rank, doc in enumerate(ranking):
            if doc in relevantdocs:
                numerator += 1
                # enumerate starts to count from 0, but for the AP we need to start from 1. Hence the +1 here.
    ...
    def mean_reciprocal_rank(self, scorefile):
        ...
        for i, ranking in enumerate(all_rankings):
            relevantdocs = all_relevantdocs[i]
            # We only compute the score for queries that actually do have duplicates in the indexed set.
            if relevantdocs != []:
    ...
    def evaluate_classification(self, scorefile):
        ...
        accuracy = self._compute_accuracy(
            truenegatives, truepositives, falsenegatives, falsepositives)
        prec_pos, prec_neg = self._compute_precision_oneclass(
            truenegatives, truepositives, falsenegatives, falsepositives)
        rec_pos, rec_neg = self._compute_recall_oneclass(
            truenegatives, truepositives, falsenegatives, falsepositives)
